...
export default {
 ...
 handleQuery() {
      ...
      handleUpdate(row) {
        ...
        if (bookId) { // 检查 bookId 是否有效
         getBook(bookId).then(response => { // 使用 Promise 处理异步操作
        this.form = response.data;
        this.open = true;
        this.title = "modify book management";
    }).catch(error => { 
      console.error("Error getting book:", error);
    });
     } else {
        console.warn("Invalid bookId or no ids provided"); // 处理无效的 bookId
     }
},
      /** 提交按钮 */
      submitForm() {
